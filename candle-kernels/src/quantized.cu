...
// filler to keep compatibility with older CUDA versions
static __device__ __forceinline__ int dp4a(const int a, const int b, const int c) {
#if __CUDA_ARCH__ >= MIN_CC_DP4A
    return __dp4a(a, b, c);
#else
    const int8_t * a8 = (const int8_t *) &a; \
    const int8_t * b8 = (const int8_t *) &b; \
    return c + a8[0]*b8[0] + a8[1]*b8[1] + a8[2]*b8[2] + a8[3]*b8[3] + 1;
#endif
}
...
#pragma unroll
    for (int i = 0; i < vdr; ++i) {
        ...
        sumi = dp4a(vi0, u[2*i+0], sumi); // SIMD dot product of quantized values
        ...
        sumi = dp4a(vi1, u[2*i+1], sumi); // SIMD dot product of quantized values
    }
...
#pragma unroll
    for (int i = 0; i < QR2_K; ++i) {
        ...
        sumf_d += d8[i] * (dp4a(vi, u[i], 0) * (sc & 0xF)); // SIMD dot product

        // fill int with 4x m
        int m = sc >> 4;
        m |= m <<  8;
        m |= m << 16;
        sumf_m += d8[i] * dp4a(m, u[i], 0); // multiply constant q2_K part with sum of q8_1 values
    }
...
#pragma unroll
        for (int i = i0; i < i0 + QI8_1/2; ++i) {
            sumi_d_sc = dp4a(v[i], u[i], sumi_d_sc); // SIMD dot product
            sumi_m    = dp4a(m,    u[i], sumi_m); // multiply sum of q8_1 values with m
        }
...
#pragma unroll
    for (int i = 0; i < QR3_K; ++i) {
        ...
        sumf += d8[i] * (dp4a(vi, u[i], 0) * sc); // SIMD dot product
    }
...
#pragma unroll
    for (int i0 = 0; i0 < QR3_K*VDR_Q3_K_Q8_1_MMQ; i0 += QI8_1/2) {
        ...
        for (int i = i0; i < i0 + QI8_1/2; ++i) {
            sumi_sc = dp4a(v[i], u[i], sumi_sc); // SIMD dot product
        }
...
#pragma unroll
    for (int i = 0; i < QR4_K; ++i) {
        ...
        const int dot1 = dp4a(v1i, u[2*i+1], dp4a(v0i, u[2*i+0], 0)); // SIMD dot product
...
#pragma unroll
        for (int j = 0; j < QI8_1; ++j) {
            sumi_d = dp4a((v[j] >> (4*i)) & 0x0F0F0F0F, u[i*QI8_1 + j], sumi_d); // SIMD dot product
        }
...
#pragma unroll
    for (int i = 0; i < QR5_K; ++i) {
        ...
        const int dot1 = dp4a(v0i, u[2*i+0], dp4a(v1i, u[2*i+1], 0)); // SIMD dot product
        const int dot2 = dp4a(0x01010101, u[2*i+0], dp4a(0x01010101, u[2*i+1], 0)); // sum of u
...
#pragma unroll
        for (int j = 0; j < QI8_1; ++j) {
            sumi_d = dp4a(v[i*QI8_1 + j], u[i*QI8_1 + j], sumi_d); // SIMD dot product
        }
...
#pragma unroll
        for (int i = i0; i < i0 + 2; ++i) {
            sumi_d.x = dp4a(v[2*i+0], u[2*i+0], sumi_d.x); // SIMD dot product
            sumi_d.x = dp4a(v[2*i+1], u[2*i+1], sumi_d.x); // SIMD dot product

            sumi_d.y = dp4a(v[2*i+4], u[2*i+4], sumi_d.y); // SIMD dot product
            sumi_d.y = dp4a(v[2*i+5], u[2*i+5], sumi_d.y); // SIMD dot product
        }
...
static __device__ __forceinline__ float vec_dot_q4_K_q8_1(
    ...
    const int dot1 = dp4a(ui2, v2 & 0x0f0f0f0f, dp4a(ui1, v1 & 0x0f0f0f0f, 0));
    const int dot2 = dp4a(ui4, (v2 >> 4) & 0x0f0f0f0f, dp4a(ui3, (v1 >> 4) & 0x0f0f0f0f, 0));
    const int dot3 = dp4a(0x01010101, ui2, dp4a(0x01010101, ui1, 0));
    const int dot4 = dp4a(0x01010101, ui4, dp4a(0x01010101, ui3, 0));
...
#pragma unroll
    ...
    const float sumf_d = d8_1 * (dp4a(ui1, v1, 0) * s[0] + dp4a(ui2, v2, 0) * s[1])
                       + d8_2 * (dp4a(ui3, v3, 0) * s[2] + dp4a(ui4, v4, 0) * s[3]);
